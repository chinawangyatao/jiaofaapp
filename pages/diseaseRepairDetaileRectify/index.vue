...
<template>
	<view class="index-wrap">
			...
			<view v-if="tabsValue === 2">
				<view class="order-state">
					...
					<view class="order-content">
						<view class="order-content-item">
							...
							<text class="content-item-value">{{diseaseData.repairCode}}</text>
						</view>
						<view class="order-content-item">
							<text class="content-item-txt">名称:</text>
							<text class="content-item-value">{{diseaseData.diseName}}</text>
						</view>
						<view class="order-content-item">
							<text class="content-item-txt">位置:</text>
							<text class="content-item-value">{{diseaseData.locationDesc}}</text>
						</view>
						<view class="order-content-item">
							<text class="content-item-txt">病害类型:</text>
							<text class="content-item-value">{{diseaseData.diseasetypeName}}</text>
						</view>
						<view class="order-content-item">
							<text class="content-item-txt">完成期限:</text>
							<text class="content-item-value">{{diseaseData.planDoneDate}}</text>
						</view>
						<view class="order-content-item">
							<text class="content-item-txt">处理要求:</text>
							<text class="content-item-value">{{diseaseData.assignRemark}}</text>
						</view>
						<view class="order-content-item">
							<text class="content-item-txt">预计工程量:</text>
							<text class="content-item-value">{{diseaseData.planQuanttityDesc}}</text>
						</view>
					</view>
				</view>
				<view class="repair-record" :style="repairRecordList.length > 0 ? 'padding-bottom: 26upx' : 'padding-bottom: 0'">
					...
					<view v-for="item in repairRecordList" :key="item.id" :class="isDailyRepair ? 'record-item-wrap' : 'record-item-wrap2'">						
						<view :style="isDailyRepair ? 'height: 120upx;': 'height: 40upx;'" class="display alert_item_center space-between">
						...
						<view v-show="item.open" style="padding-bottom: 30upx;">
							...
							<view v-if="!isDailyRepair" style="font-size: 30upx;color: #383838;display: flex;">
							...
							<view style="font-size: 30upx;color: #383838;margin-top: 30upx;">
								<!-- <main-bar-inter :name="'安全照片'" :isDis='true' :isDisBottom='false'>
									<view @click="screen">
										<upload-image-bar @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailySafeEnclosureList' @updateImage="function(list) {updateImageOne(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
									</view>
								</main-bar-inter> -->
								<view>
									<view style="margin-bottom: 26upx;margin-top: 40upx;color: #767676;">安全照片：</view>
									<upload-image-bar :width="186" @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailySafeEnclosureList' @updateImage="function(list) {updateImageOne(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
								</view>
								
								<!-- <main-bar-inter :name="'施工前照片'" :isDis='true' :isDisBottom='false'>
									<view @click="screen">
										<upload-image-bar @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailyBeforeEnclosureList' @updateImage="function(list) {updateImageTwo(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
									</view>
								</main-bar-inter> -->
								<view>
									<view style="margin-bottom: 26upx;margin-top: 40upx;color: #767676;">施工前照片：</view>
									<upload-image-bar :width="186" @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailyBeforeEnclosureList' @updateImage="function(list) {updateImageTwo(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
								</view>
								
								<!-- <main-bar-inter :name="'施工中照片'" :isDis='true' :isDisBottom='false'>
									<view @click="screen">
										<upload-image-bar @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailyMidEnclosureList' @updateImage="function(list) {updateImageThree(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
									</view>
								</main-bar-inter> -->
								<view>
									<view style="margin-bottom: 26upx;margin-top: 40upx;color: #767676;">施工中照片：</view>
									<upload-image-bar :width="186" @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailyMidEnclosureList' @updateImage="function(list) {updateImageThree(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
								</view>
								
								<!-- <main-bar-inter :name="'施工后照片'" :isDis='true' :isDisBottom='false'>
									<view @click="screen">
										<upload-image-bar @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailyAfterEnclosureList' @updateImage="function(list) {updateImageFour(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
									</view>
								</main-bar-inter> -->
								<view>
									<view style="margin-bottom: 26upx;margin-top: 40upx;color: #767676;">施工后照片：</view>
									<upload-image-bar :width="186" @deleteVideoClick="deleteVideoClick" :file='item.matRepairDailyAfterEnclosureList' @updateImage="function(list) {updateImageFour(list, item)}" :isUpdate="isUpdateFalse"></upload-image-bar>
								</view>
							</view>
							...
							<!-- <main-bar-inter :name="'施工内容：'" :vm="'180upx'">
								...
								<view style="margin-left: 58upx;">{{item.repairDailyContent}}</view>
							</main-bar-inter> -->
						...
						<!-- 	<view class="dis" style="width: 180upx;display: flex;margin-top: 42upx;">
								<view style="font-size: 16px;color: #383838;margin: 13px 0;font-size: 28upx;color: #767676;width: 220px;">施工内容：</view>
								<view style="margin-left: 58upx;">{{item.repairDailyContent}}</view>
							</view> -->
				...
				<view class="repair-record-bottom">
					...
					<view v-if="bottomTabsValue === 1" style="box-sizing: border-box;padding-left: 24upx;padding-right: 24upx;">
						<!-- <view style="display: flex;margin-top: 42upx;">
							...
							<view style="font-size: 28upx;color: #383838;margin-left: 8upx;">{{contractName}}</view>
						</view>
						<view style="display: flex;margin-top: 24upx;">
							<view style="font-size: 28upx;color: #767676;">清单：</view>
							<view style="font-size: 28upx;color: #383838;margin-left: 24upx;">{{checklistName}}</view>
						</view> -->
						
						<view style="display: flex;margin-top: 42upx;">
							<view style="width: 90upx;font-size: 30upx;color: #767676;">合同：</view>
							<view style="font-size: 30upx;color: #383838;width:560upx;">{{contractName}}</view>
						</view>
						<view style="display: flex;margin-top: 24upx;">
							<view style="width: 90upx;font-size: 30upx;color: #767676;">清单：</view>
							<view style="font-size: 30upx;color: #383838;width:560upx;">{{checklistName}}</view>
						</view>
					...
					<view v-if="bottomTabsValue === 2" style="box-sizing: border-box;padding-left: 24upx;padding-right: 24upx;">
						<view class="engineer-item" v-for="item in addBaseMaterialList">
				...
				<view class="order-state-two" v-if="this.transitionList.length > 0">
				...
				<view class="order-state-two">
					...
					<view style="padding-left: 24upx;width: 654upx;">
						<main-bar-inter :name="'整改建议：'" :vm="'190upx'" :isDisBottom='false'>
							<!-- <input /> -->
							<view style="width: 480upx;">{{reformData.reformRemark}}</view>
						</main-bar-inter>
					</view>
					...
					<view style="font-size: 30upx;color: #383838;margin-left: 24upx;margin-top: 20upx;">
						<!-- <main-bar-inter :name="'整改后照片'" :isDis='true' :isDisBottom='false'>
							<view @click="screen">
								<upload-image-bar @deleteVideoClick="deleteVideoClick" :file='reformData.matRepairRectificationEnclosureListT' @updateImage="updateImage" :isUpdate="isUpdate"></upload-image-bar>
							</view>
						</main-bar-inter> -->
						<view>
							<view style="margin-bottom: 26upx;margin-top: -20upx;">整改后照片：</view>
							<!-- <view class="file">整改后照片</view> -->
							<upload-image-bar :width="186" @deleteVideoClick="deleteVideoClick" :file='reformData.matRepairRectificationEnclosureListT' @updateImage="updateImage" :isUpdate="isUpdate"></upload-image-bar>
						</view>
					</view>
					<view style="padding-left: 24upx;width: 654upx;">
						<main-bar-inter :name="'整改内容：'" :vm="'190upx'">
							<input placeholder="请输入" style="width: 400upx;" v-model='reformContent'/>
						</main-bar-inter>
					</view>
				</view>
			</view>
		...
		<view v-if="tabsValue === 1" class="display alert_item_center bottom-btn-wrap">
			<view class="btn-item" style="height: 78upx;border: 1upx solid #3695e2;color: #1890FF;background: #fff;" @click='saveRectify(false)'>保存</view>
			<view class="btn-item" style="margin-left: 30upx;color: #fff;background: #1890FF;" @click='saveRectifyCheck()'>提交</view>
		</view>
...
<script>
	...
	export default{
		components: {
			TabsBar, MainBarInter, RadionButtonBar, uploadImageBar, NTransition, CloneBar
		},
		data(){
			return {
				name: '维修详情',
		...
		methods:{
			getItemDetaile(params) {
			    let _this = this
			    _this.$api.send({
			         url: `/repair/matRepair/getRectificationByRepairId?id=${params.id}`,
			         method: 'get',
			         success:(res)=>{		          
						 
						_this.reformData = res.data
						_this.$set(_this.reformData, 'matRepairRectificationEnclosureListT', JSON.parse(JSON.stringify(res.data.matRepairRectificationEnclosureList)))
						//被要求整改前的正在验收用户S(如果是谁驳回，整改提交再由谁验收，打开if下的注释)
						let acceUser = res.data.acceUser
						if (acceUser) {						
							//_this.getRoleListByUserId(acceUser)
						}						
						//E
					 },
			         fail:(res)=>{
				...
				_this.$api.send({
				     url: `/repair/matRepair/getDiseaseMessageAndRectification?id=${params.id}`,
				     method: 'get',
				     success:(res)=>{
						 //alert(res.data.matDisease.supervisorUnitId)
						//console.log(res,"+++++++++++++2")
						if (res.data.id) {
							_this.getLog(res.data.id,res.data.diseaseId)
						}
						_this.diseaseData = res.data
						_this.reformContent = res.data.reformAfterRemark ? res.data.reformAfterRemark : ''
						...
						// _this.acceptanceList = res.data.matRepairAccEnclosureList ? res.data.matRepairAccEnclosureList : []
			...
			getRoleListByUserId(params) {
				if(params){
					let _this = this
					_this.$api.send({
					     url: `/sys-auth/user/getRoleListByUserId?userId=${params}`,
					     method: 'get',
					     success:(res)=>{							
							  //////
							 if(res.data){
								 res.data.map(item => {
									if(item.code){
										_this.userRoleList.push(item.code)
									}								 	
								 })
							 }
							  /////////
			...
			deleteVideoClick(index) {
				this.reformData.matRepairRectificationEnclosureList[index].modifyType = 2//'2'
				//this.reformData.matRepairRectificationEnclosureListT.splice(index, 1)
				this.reformData.matRepairRectificationEnclosureListT[index].modifyType = 2
			},
			updateImage(fileList) {
				this.reformData.matRepairRectificationEnclosureList = fileList
				this.reformData.matRepairRectificationEnclosureListT = fileList
			},
			saveRectifyCheck() {
				console.log(this.reformData.matRepairRectificationEnclosureList)
				console.log(this.reformContent)
				if (this.reformData.matRepairRectificationEnclosureList && this.reformData.matRepairRectificationEnclosureList.length < 1) {
					uni.showToast({
						title: '请上传整改照片',
						icon: 'none',
						duration: 3000
					})
				} else if (this.reformContent.toString().trim().length < 1) {
					uni.showToast({
						title: '请填写整改内容',
						icon: 'none',
						duration: 3000
			...
			saveRectify(flag) {
				if (this.diseaseData.reformAfterRemark) {
					this.diseaseData.reformAfterRemark = this.reformContent
				} else {
					this.$set(this.diseaseData, 'reformAfterRemark', this.reformContent)
				}
				if (this.diseaseData.matRepairRectificationEnclosureList) {
					this.diseaseData.matRepairRectificationEnclosureList = this.reformData.matRepairRectificationEnclosureList
				} else {
					this.$set(this.diseaseData, 'matRepairRectificationEnclosureList', this.reformData.matRepairRectificationEnclosureList)
				}
				//////
				//被要求整改前的正在验收用户S
				if(flag){//提交
					if(this.userRoleList && this.userRoleList.length != 0){
						let roleCode = this.userRoleList[0]
						let status1 = roleCode===jurisdictionList[3] ? '7' : roleCode===jurisdictionList[1] ? '8' : '9'	
						let status1_text = roleCode===jurisdictionList[3] ? '监理审批' : roleCode===jurisdictionList[1] ? '验收审核' : '验收审批'
						this.diseaseData.status = status1
						this.diseaseData.status_text = status1_text	
					}else{
						//整改总是从头开始
						if(this.selectUser){					
							//如果有监理验收
							//this.diseaseData.status = '7'
							//this.diseaseData.status_text = '监理审批'
							this.diseaseData.status = '8'
							this.diseaseData.status_text = '验收审核'
							this.$set(this.diseaseData, 'acceUser', this.selectUser.id)
						}		
					}//E
				}				
				console.log(this.diseaseData)
				let _params = this.diseaseData
				let _this = this
				_this.$api.send({
